...
![img](https://cdn.nlark.com/yuque/0/2021/png/2923566/1612453993167-e5640a4a-1ac1-4f29-9b00-ee28af789512.png)
...
JVM内存模型规定了每个区域的生命周期和作用，同时也定义了JVM如何管理内存。开发人员可以根据JVM内存模型来优化序的性能和内存占用。 JVM 内存模型共分为5个区：Java虚拟机栈、本地方法栈、堆、程序计数器、方法区（元空间）

![img](https://cdn.nlark.com/yuque/0/2023/png/2923566/1682414824720-11c2bdd7-995a-43de-89d7-a6c44be43d08.png)

## 各个区各自的作用

### 本地方法栈

用于管理本地方法的调用，里面并没有我们写的代码逻辑，其由native修饰，由 C 语言实现。 与Java虚拟机栈类似，但是用于保存本地方法调用的栈。
...
用于保存类的结构信息、常量池、静态变量、即时编译器编译后的代码等。

### Java 虚拟机栈

用于存储局部变量表、操作数栈、动态链接、方法出口等信息。（栈里面存的是地址，实际指向的是堆里面的对象） 用于保存每个线程的方法调用栈。每当一个方法被调用时，JVM就会在栈中为该方法创建一个新的栈帧，用于保存该方法的参数、局部变量和运行时数据。

### 堆

Java 虚拟机中内存最大的一块，是被所有线程共享的，几乎所有的对象实例都在这里分配内存 用于保存对象实例和数组。堆内存由JVM自动管理，可以动态分配和回收。

## 线程私有、公有

- 线程私有：每个线程在开辟、运行的过程中会单独创建这样的一份内存，有多少个线程可能有多少个内存 Java虚拟机栈、本地方法栈、程序计数器是线程私有的
- 线程全局共享的: 堆和方法区 栈虽然方法运行完毕了之后被清空了，但是堆上面的还没有被清空，所以引出了GC（垃圾回收），不能立马删除，因为不知道是否还有其它的也是引用了当前的地址来访问的

# JAVA内存模型

以下问答均由[AI聊天助手](https://gulong.tech/openai/chat)生成

## java内存模型和jvm内存模型是同一个东西吗

不完全相同。Java内存模型（Java Memory Model，简称JMM）是用于规范Java程序中多线程并发访问共享变量时，各个线程之间如何进行通信和同步的一套规范。而JVM内存模型（JVM Memory Model）则是用于管理JVM内存的一套规范，它定义了JVM如何将内存划分为不同的区域，并规定了每个区域的作用和生命周期。

Java内存模型和JVM内存模型虽然不是同一个东西，但是它们之间有一定的关联。Java程序中多线程并发访问共享变量时，需要按照Java内存模型中的规范进行通信和同步，而这些操作会影响到JVM内存模型中的某些区域，比如堆内存和线程栈等。因此，在编写Java程序时，需要同时考虑Java内存模型和JVM内存模型的规范。

## 介绍一下java内存模型

Java内存模型（Java Memory Model，简称JMM）是用于规范Java程序中多线程并发访问共享变量时，各个线程之间如何进行通信和同步的一套规范。Java内存模型主要解决的问题是如何保证多线程程序中共享变量的可见性、有序性和原子性。

Java内存模型围绕着原子性、可见性和有序性这三个概念展开：

1. 原子性：原子性指的是一组相关操作被视为一个不可分割的整体，要么全部执行成功，要么全部不执行。在Java中，所有的基本类型的读取和赋值操作（除了long和double类型）都是原子性的，但是涉及到多个操作的复合操作却不具有原子性。
2. 可见性：可见性指的是当一个线程对共享变量做出修改后，其他线程能够立刻看到该修改。如果不采取特殊的措施，Java中的共享变量对所有线程都是可见的。
3. 有序性：有序性指的是程序执行的顺序与代码中的顺序一致。在Java中，由于编译器和处理器会对代码进行优化，因此程序执行的顺序可能与代码中的顺序不完全一致。 为了解决这些问题，Java内存模型规定了以下几个重要的概念：
4. 主内存（Main Memory）：主内存是所有线程共享的内存区域，Java中的所有变量都存储在主内存中。
5. 工作内存（Working Memory）：每个线程都有自己的工作内存，线程的工作内存保存了该线程使用到的变量的副本。
6. 内存屏障（Memory Barrier）：内存屏障是一种同步机制，用于保证各个线程之间的数据同步和顺序执行。
7. 锁（Lock）：锁是一种同步机制，用于保证同一时间只有一个线程能够访问共享资源。 Java内存模型以及相关的同步机制，可以保证多线程程序中的共享变量能够正确地实现原子性、可见性和有序性，从而保证了程序的正确性。
...
1. 请列举一些常见的Java GC算法以及它们的优缺点。
2. 什么是垃圾收集器？请列举一些常见的Java垃圾收集器以及它们的优缺点。
3. 如何判断一个Java对象是否可以被垃圾收集器回收？
4. 请简述Java对象的生命周期。
5. 什么是内存泄漏？如何避免它？
6. 什么是堆和栈？它们之间的区别是什么？
7. 如何调整Java的堆大小？
8. 什么是永久代？在Java 8中，它被什么所取代？
9. 什么是GC日志？如何分析GC日志以及解决GC问题？

## 什么是Java GC？为什么需要它？

Java GC（Garbage Collection）是Java虚拟机（JVM）用于管理和回收无用对象的机制。在Java中，程序员无法直接控制内存的分配和释放，而GC负责在程序运行时自动地从内存中清除不再使用的对象以释放内存空间。

需要Java GC的主要原因是，当程序运行时，需要频繁地动态分配和释放内存空间来存储对象。如果没有GC，则程序员需要手动管理内存的分配和释放，这会增加代码的复杂性和出错的可能性。此外，如果程序员忘记释放一个对象的内存空间，则会导致内存泄漏的问题，最终耗尽内存并导致程序崩溃。

因此，Java GC的主要作用是自动地回收无用对象的内存空间，使程序员可以更专注于编写业务逻辑，而不必担心内存管理的问题。

## 什么是垃圾收集器？请列举一些常见的Java垃圾收集器以及它们的优缺点。

垃圾收集器是Java虚拟机中负责执行GC的组件，它们根据不同的算法来搜索和回收无用对象，以释放内存空间。

以下是一些常见的Java垃圾收集器以及它们的优缺点：

1. Serial垃圾收集器：Serial垃圾收集器是一种单线程的垃圾收集器，它使用标记-清除算法来搜索和回收无用对象。它的优点是简单、高效，适用于小型应用程序，但缺点是不能充分利用多核CPU，不能与应用程序并行执行。
2. Parallel垃圾收集器：Parallel垃圾收集器是一种多线程的垃圾收集器，它使用标记-清除算法来搜索和回收无用对象。它的优点是可以利用多核CPU，加快垃圾收集速度，适用于需要高吞吐量的应用程序，但缺点是在垃圾收集时会占用大量的CPU资源，可能影响应用程序的响应速度。
3. CMS垃圾收集器：CMS（Concurrent Mark Sweep）垃圾收集器是一种并发的垃圾收集器，它使用标记-清除算法来搜索和回收无用对象。它的优点是可以与应用程序并行执行，不会占用太多的CPU资源，适用于需要低暂停时间的应用程序，但缺点是可能会导致碎片化问题，对于大型堆空间的应用程序，可能会导致Full GC的频繁发生。
4. G1垃圾收集器：G1（Garbage-First）垃圾收集器是一种优化的垃圾收集器，它针对大型堆空间的应用程序进行优化，可以将堆空间分成多个区域，进行优先回收。它的优点是可以在短时间内回收大量无用对象，减少Full GC的发生，适用于大型堆空间的应用程序，但缺点是相比于其他垃圾收集器，它的实现较为复杂。

以上是一些常见的Java垃圾收集器以及它们的优缺点，选择哪种垃圾收集器取决于应用程序的需求和性能要求。

## 如何判断一个Java对象是否可以被垃圾收集器回收？

Java对象是否可以被垃圾收集器回收，取决于它是否被引用。

在Java中，如果一个对象没有任何引用指向它，那么它就可以被垃圾收集器回收。具体来说，以下情况下对象可以被回收：
...
需要注意的是，即使一个对象已经不再被使用，但由于存在循环引用等情况，它仍然可能无法被垃圾收集器回收。因此，在编写代码时，应该尽量避免循环引用和不必要的对象引用。

## 请简述Java对象的生命周期。

Java对象的生命周期可以分为以下几个阶段：

- 创建阶段：在Java程序中，通过new操作符或反射等方式创建一个新的对象。在这个阶段，Java虚拟机会为对象分配内存空间，并初始化对象的成员变量。
- 使用阶段：在Java程序中，程序员可以通过对象引用来操作该对象。在使用阶段，对象可能被传递给其他对象或方法，也可能被存储在集合中等等。
- 不可达阶段：如果某个对象没有任何引用指向它，则它就进入了不可达阶段。当对象处于不可达状态时，垃圾收集器会在某个时刻将其回收。
- 回收阶段：在Java程序中，垃圾收集器会定期扫描堆内存，查找不再使用的对象，并回收它们的内存空间。在回收阶段，对象的内存空间被释放，并且对象被从Java程序中删除。

需要注意的是，Java对象的生命周期并不是线性的，对象可能会在使用阶段被引用多次，进入和退出不可达阶段多次，最终被垃圾收集器回收。

## 什么是内存泄漏？如何避免他？

内存泄漏是指程序中的某些对象占用了内存空间，但却没有被回收，导致内存空间的浪费和程序性能的下降。

内存泄漏的原因可能包括以下几个方面：

对象没有被正确地释放：如果程序员忘记释放一个对象的内存空间，或者释放时出现了异常而没有完成释放操作，那么这个对象就会一直占用内存空间，导致内存泄漏。

- 长时间运行的程序：如果程序长时间运行，同时创建了大量的对象，那么这些对象可能会导致内存泄漏，因为它们会一直占用内存空间。
- 循环引用：如果两个或多个对象之间存在循环引用，那么它们都无法被回收，导致内存泄漏。

为了避免内存泄漏，可以采取以下几个措施：

- 显式地释放对象：程序员可以手动调用对象的dispose()或close()等方法来释放对象占用的内存空间。
- 使用try-with-resources：如果对象实现了AutoCloseable接口，程序员可以使用try-with-resources语句块来自动释放对象的资源。
- 避免创建过多的对象：程序员可以尽量减少对象的创建，避免创建过多的对象导致内存泄漏。
- 避免循环引用：程序员可以尽量避免循环引用的情况出现，比如使用弱引用或软引用等。

需要注意的是，内存泄漏可能会导致程序的性能下降，甚至导致程序崩溃。因此，在编写Java程序时，需要注意内存泄漏的问题，并采取相应的措施进行预防。

## 什么是堆和栈？它们之间的区别是什么？

堆和栈是Java虚拟机中两种重要的内存区域，它们之间有以下区别：

1. 内存结构不同：堆是用于存储对象的内存区域，栈是用于存储程序执行时的临时数据和方法调用的内存区域。
2. 空间大小不同：堆的空间大小是动态分配的，可以根据程序的需要进行扩展或缩小，而栈的空间大小是固定的，一旦分配后无法改变。
3. 内存分配方式不同：堆的内存分配是由垃圾收集器自动完成的，程序员无法直接控制；而栈的内存分配是由Java虚拟机自动分配和回收的，程序员只需要关心如何使用栈上的数据即可。
4. 对象的存储方式不同：在堆中分配的对象存储的是对象的引用地址，而实际的对象数据存储在堆之外；而在栈中分配的数据存储的是实际的数据，而不是引用地址。

需要注意的是，Java虚拟机还包括其他的内存区域，比如方法区、常量池等，这些内存区域也和堆、栈有所不同。在编写Java程序时，需要了解内存区域的不同，以便对内存进行合理的管理和优化。

## 如何调整Java的堆大小？

Java堆的大小是通过JVM参数来进行设置的。可以使用以下两个参数来设置Java堆的大小：

1. -Xms：设置初始堆大小。如果没有设置-Xmx，则-Xms同时也表示最大堆大小。
2. -Xmx：设置最大堆大小。

这两个参数可以用来控制Java程序的性能和内存使用情况。如果Java程序需要处理大量的数据，可以适当增大堆的大小以提高程序的性能。如果Java程序只需要处理少量的数据，可以适当减小堆的大小以节省内存空间。

例如，如果要将初始堆大小设置为512MB，最大堆大小设置为1024MB，可以使用以下命令行参数：

-Xms512m -Xmx1024m 需要注意的是，堆大小的调整应该根据程序的实际需求进行设置，并且设置过大或过小都可能会导致性能和内存问题。因此，在进行堆大小的调整时，需要进行充分的测试和评估，以确保程序的正常运行和性能优化。

## 什么是永久代？在Java 8中，它被什么所取代？

永久代是Java虚拟机中的一块内存区域，用于存储类的元数据、静态变量和常量等数据。在Java 7及之前的版本中，永久代的大小是固定的，不能随着程序的运行而动态调整。

在Java 8中，永久代被元空间（Metaspace）所取代。元空间是一块类似于永久代的内存区域，它可以动态调整大小，并且可以利用本地内存（Native Memory）来存储元数据和其他数据，减少了对Java堆的使用。

与永久代不同，元空间的大小是由操作系统分配的本地内存来控制的，当元空间的内存不足时，会触发垃圾回收（GC）进行元数据的清理和释放。元空间的使用方式也与永久代不同，Java虚拟机提供了一些参数来控制元空间的大小和使用行为，如-XX:MetaspaceSize、-XX:MaxMetaspaceSize等。

相对于永久代，元空间具有以下优点：

1. 可以动态调整大小，避免了永久代的固定大小导致的内存不足问题。
2. 可以利用本地内存，减少了对Java堆的使用，提高了程序的性能。
3. 支持class metadata sharing，可以在多个Java虚拟机之间共享元数据，提高了程序的启动速度和内存利用率。 需要注意的是，在Java 8之前的版本中，需要通过调整JVM参数来控制永久代的大小和使用行为。而在Java 8及之后的版本中，永久代已经被元空间所取代，因此不再需要调整永久代相关的JVM参数。

## 什么是GC日志？如何分析GC日志以及解决GC问题？

GC日志是Java虚拟机在运行时记录的垃圾收集信息，包括垃圾收集的时间、垃圾收集器的类型、GC前后的内存使用情况等信息。通过分析GC日志，可以了解垃圾收集器的工作情况和Java程序的内存使用情况，从而优化程序的性能。

在Java程序中，可以通过以下JVM参数来启用GC日志的记录：

1. -verbose:gc：打印GC日志信息。
2. -Xloggc:：将GC日志信息写入指定的文件中。

在生成GC日志后，可以使用以下工具来分析GC日志：

1. jstat：用于监控Java虚拟机的内存使用情况和垃圾收集情况。
2. jmap：用于生成Java程序的内存快照，以便进行内存泄漏分析。
3. jhat：用于将Java程序的内存快照转换为可视化的HTML格式，方便进行内存泄漏分析。

GCViewer：用于可视化GC日志信息，方便进行垃圾收集器的性能分析和优化。

在分析GC日志时，需要注意以下几点：

1. 分析GC日志需要一定的经验和技巧，需要掌握Java虚拟机垃圾收集的相关知识。
2. 需要结合程序的实际情况进行分析，比如程序的内存使用情况、垃圾收集器的类型和配置等。
3. 需要针对具体的问题进行调整和优化，比如增加堆的大小、调整垃圾收集器的参数等。

需要注意的是，GC日志分析和优化是一个复杂的过程，需要根据实际情况进行调整和优化，建议结合实际情况和专业人员的建议进行分析和优化。
...
![img](https://cdn.nlark.com/yuque/0/2021/png/2923566/1614174536013-cffb356c-5ad8-4fa5-a457-a174dc734062.png)
...
![img](https://cdn.nlark.com/yuque/0/2021/jpeg/2923566/1614174536032-f4986c04-069a-4017-8ba2-320880c099c7.jpeg)
...
![img](https://cdn.nlark.com/yuque/0/2021/png/2923566/1614174536020-d9dcf8ec-a5fc-40ad-8151-45d8aae47380.png)

![img](https://cdn.nlark.com/yuque/0/2021/png/2923566/1614174536049-5e4eaad7-49cc-4b2e-a497-e318199fb702.png)

### 标记清除

![img](https://cdn.nlark.com/yuque/0/2021/png/2923566/1614174536019-f41b0710-f1fc-4958-8331-edf4cb5109c9.png)

![img](https://cdn.nlark.com/yuque/0/2021/png/2923566/1614174536022-aa5c93f2-1a02-4f70-a423-4e45d4e8c05a.png)

### 复制算法

![img](https://cdn.nlark.com/yuque/0/2021/png/2923566/1614174536010-54c8d94b-22a9-498a-a4f4-5821c2140c48.png)

![img](https://cdn.nlark.com/yuque/0/2021/png/2923566/1614174536008-f765109c-8df5-41be-b97e-7b020be30744.png)
...
![img](https://cdn.nlark.com/yuque/0/2021/png/2923566/1614174536052-251b9c08-2477-4d6b-9273-a9073a2c908e.png)

连线表示可以搭配使用，红叉表示不推荐一同使用，比如新生代用`Serial`，老年代用`CMS`。

![img](https://cdn.nlark.com/yuque/0/2021/png/2923566/1614174536033-d0023168-69bf-49ed-9f98-24feee61f4dd.png)
...
![img](https://cdn.nlark.com/yuque/0/2021/jpeg/2923566/1614174536004-5bf20e42-0270-4535-8a6b-9f76fae7d155.jpeg)
...
![img](https://cdn.nlark.com/yuque/0/2021/jpeg/2923566/1614174536017-e6705e4b-dfe8-40e3-8944-c9b7908e0c33.jpeg)
...
![img](https://cdn.nlark.com/yuque/0/2021/jpeg/2923566/1614174536804-b9f3a991-7e36-4eef-a2ed-cd4ba09f136a.jpeg)
...
[![img](https://cdn.nlark.com/yuque/0/2021/png/2923566/1614174536036-fbc34de6-a43d-4b21-a53c-00893579428b.png)](https://github.com/lvminghui/Java-Notes/blob/master/docs/imgs/JVM.png)
...
jmeter 压力测试 ， Undertow （比tomcat QPS 每秒大1000-3000）

jinfo -flags pid 查看jvm参数

top /uptime 看整机性能 -》   load average
...
- 方法区
  用于存放已被加载的类信息、常量、静态变量、即时编译器编译后的代码等数据。方法区是一个 JVM 规范，永久代与元空间都是其一种实现方式。
- 对方法区 和 永久区的理解，以及他们之间的关系 方法区是JVM 里面的规范要求， 永久区是Hotspot 虚拟机对方法区的具体实现方式 ，类似的有JDK1.8 引入的元空间

### 堆

堆内存模型：

![img](https://gitee.com/houyao123/my-resource/raw/master/img/20210303120820.png)
...
![img](https://gitee.com/houyao123/my-resource/raw/master/img/20210303120650.png)
...
[![img](https://gitee.com/houyao123/my-resource/raw/master/img/20210303120753.png)](https://github.com/lvminghui/Java-Notes/blob/master/docs/imgs/JVM.png)
